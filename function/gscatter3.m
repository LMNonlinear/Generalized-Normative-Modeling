...
if ~exist('mrksle','var')||isempty(mrksle),
    ...
    if length(mrksle)==length(g),
        ...
        if length(mrksle)==1
            mrksle=repmat(mrksle,1,length(unique(gu)));
        end
        if ischar(mrksle),
...
for i=1:length(gu),
    ...
    if ~iscellstr(clr)&&~ischar(clr),
        ...
        set(h(1,i),'LineStyle','none','linewidth',1,'Color',clr{1,i},'Marker',mrksle{1,i},'MarkerSize',mrksze,'MarkerFaceColor',mrkfclr{1,i});
    end;
end;
